...
void *man(void *arg) {
  ...
  int randomTime = rand() % 1000001; 
  usleep( randomTime ); // tempo aleátorio dentro do banheiro

  // Adquire o lock novamente para sair do banheiro
  pthread_mutex_lock(&mutex);
  printf("Homem saindo do banheiro\n");
  bathroomSpots++;
  peopleThatNeedToGoToTheBathroom--; // Quando a pessoa sai do banheiro, marcamos isso para o contador
...
void *woman(void *arg) {
  ...
  while(needsToGoToTheBathroom) {
    ...
    womanSwitch = 1;
    if (bathroomSpots > 0) {
      printf("Há espaço no banheiro. Mulher ocupando-o...\n");
      bathroomSpots--;
      needsToGoToTheBathroom = 0;
    }
    pthread_mutex_unlock(&mutex);
...
int main(void) {
  ...
  pthread_t men[20];
  pthread_t women[20];
  pthread_t bathroomThread;
  // Criando Thread para o banheiro
  pthread_create(&bathroomThread, NULL, bathroom, NULL);
  ...
  // Programa só finaliza quando terminar a execução da thread do banheiro (todo mundo foi)
  pthread_join(bathroomThread, NULL);
  return 0;
}
